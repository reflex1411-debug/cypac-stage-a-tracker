...
# Direct import from db module (No circular imports)
from db import db
...
st.caption(
    f"System Operational Date: {today_str} | Clinical Audit & Quality Assurance"
    " Matrix"
)
st.divider()
    ...
    with st.form("stage_a_form", clear_on_submit=True):
        ...
        with col1:
            clinician_name = st.text_input(
                "Clinician / Tester Name:", placeholder="e.g., J. Smith"
            )
            ...
            equipment_id = st.text_input(
                "Equipment / Audiometer Serial No:", placeholder="e.g., AUD-2026-X"
            )

        with col2:
            visual_check = st.selectbox(
                "Visual Inspection (Cables, Headphones, Transducers):",
                ["PASS", "FAIL"],
            )
            listening_check = st.selectbox(
                "Listening Check (Signal Purity & Frequency Accuracy):",
                ["PASS", "FAIL"],
            )
            overall_status = (
                "PASS"
                if (visual_check == "PASS" and listening_check == "PASS")
                else "FAIL"
            )
        ...
        notes = st.text_area(
            ...
            placeholder=(
                "Record any acoustic irregularities, wear, or fault details"
                " here..."
            ),
        )
        ...
        if submitted:
            if not clinician_name or not equipment_id:
                st.error(
                    "Please enter both Clinician Name and Equipment Serial"
                ...
                if overall_status == "FAIL":
                    db.log_fault(
                        ...
                        fault_description=(
                            "Stage A Fail:"
                            f" {notes if notes else 'Visual/Listening check failed.'}"
                        ),
                        check_id=check_id,
                        reported_by=clinician_name,
                    )
                    st.warning(
                        f"Check #{check_id} saved as FAIL. An open fault ticket"
                        " has been automatically logged."
                    )
                else:
                    st.success(
                        f"Stage A Check #{check_id} successfully saved as"
                        " PASS!"
                    )
                st.rerun()
...
with tab_faults:
    ...
    if df_faults.empty:
        st.success(
            "All room equipment is fully operational! No active faults"
            " currently logged."
        )
    else:
        ...
        fault_to_resolve = st.selectbox(
            "Select Fault ID to Resolve:", df_faults["fault_id"].tolist()
        )
        ...
        if st.button("✅ Mark Selected Fault as Resolved"):
            if not resolution_notes:
                st.error(
                    "Please provide resolution notes before closing the ticket."
                )
            else:
...
with tab_audit:
    ...
    with db.get_connection() as conn:
        df_audit = pd.read_sql_query(
            "SELECT * FROM audit_logs ORDER BY timestamp DESC LIMIT 50", conn
        )
